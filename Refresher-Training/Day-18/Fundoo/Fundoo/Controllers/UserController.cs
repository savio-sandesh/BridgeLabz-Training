...
namespace Fundoo.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [Authorize]
        [HttpPost("logout")]
        public async Task<IActionResult> Logout([FromServices] IRedisCacheService redisCacheService)
        {
            var authHeader = Request.Headers["Authorization"].FirstOrDefault();
            if (string.IsNullOrEmpty(authHeader) || !authHeader.StartsWith("Bearer ", StringComparison.OrdinalIgnoreCase))
            {
                return BadRequest(new { message = "Invalid or missing Authorization header." });
            }

            var token = authHeader.Substring("Bearer ".Length).Trim();

            // Token ko Redis blacklist me dalte hain with 60 mins expiry
            await redisCacheService.SetTokenAsync(token, TimeSpan.FromMinutes(60));
