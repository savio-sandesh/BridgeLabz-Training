...
A Google Keep–inspired note-taking backend built with ASP.NET Core Web API, following a layered (multi-project) architecture with JWT-based authentication, Cloudinary media handling, label management, RabbitMQ-driven reminders, Redis token caching, and automated MSTest unit test coverage.
...
Fundoo Notes App is being built as a multi-day training project, progressively growing from a simple User Management module into a full enterprise-grade notes application (registration, notes CRUD, pinning/archiving, labels/tags management, Cloudinary CDN uploads, RabbitMQ-based reminders, Redis token caching, trash lifecycle, and unit test suites). This README reflects progress as of the **User Management + Notes Core & Media + Lifecycle + Labels Management + Reminders (RabbitMQ) + Redis Token Caching + MSTest Suite** milestone.
...
| In-Memory Cache / Revocation | Redis / Upstash (`StackExchange.Redis`) |
...
```
[HTTP Request with Bearer Token] ──► [TokenBlacklistMiddleware] ──► [Redis In-Memory Cache (~0.5ms)]
                                               │
                                       (If Valid Token)
                                               ▼
...
- **BusinessLayer** — Business workflows, password hashing, JWT token generation, Cloudinary media processing, `IRabbitMqProducer`/`RabbitMqProducer` publishing logic, `IRedisCacheService`/`RedisCacheService` implementation for token blacklisting, and label/note business rules.
- **Fundoo (WebAPI Host)** — Controllers, route guards (`[Authorize]`), middleware pipeline (including `TokenBlacklistMiddleware`, which rejects requests carrying blacklisted/revoked tokens before they reach a controller), DI registration, and the `ReminderNotificationConsumer` `BackgroundService` (`IHostedService`) that listens for and consumes queued reminder events.
...
- **Redis / Upstash Cache** — Backs instant, stateless JWT revocation. On logout, the token is written to Redis with a TTL; `TokenBlacklistMiddleware` checks every authenticated request against this cache (sub-millisecond, no database hit) to reject revoked tokens immediately.
## Project Structure
...
### Token Caching & Revocation via Redis (Day 18)
- **Instant Stateless JWT Invalidation:** User logout par token ko Redis in-memory cache me with TTL (Time To Live) blacklist kiya jata hai.
- **Sub-Millisecond Verification:** `TokenBlacklistMiddleware` har authenticated request ko database hit kiye bina (< 1ms) check karta hai ki token revoked toh nahi hai.
...
| POST | `/api/v1/user/logout` | **Yes** | Invalidate JWT token and blacklist via Redis |
...
- Upstash Redis Instance (Cloud Free Tier) or Local Redis / Docker
### Setup & Run
...
Connection strings, JWT secrets, SMTP credentials, Cloudinary API keys, RabbitMQ, and Redis connection details are managed in `Fundoo/appsettings.json`:
...
```json
{
  ...
  "Redis": {
    "ConnectionString": "<your-endpoint>.upstash.io:6379,password=<password>,ssl=True,abortConnect=False"
  },
  "SmtpSettings": {
...
4. **Logout:** Call `POST /api/v1/user/logout` with the Bearer Token → `200 OK` (token blacklisted in Redis).
5. **Verify Revocation:** Call any protected endpoint (e.g. `GET /api/Notes`) with the same token → `401 Unauthorized` (token has been revoked). Log in again to obtain a fresh token before continuing.
### 2. Core Notes Operations
6. **Create Note:** Send `POST /api/Notes` with `{ "title": "Meeting Notes", "description": "Discuss architecture", "backgroundcolor": "#FFE4C4" }`. Note down the returned `noteId`.
7. **Get Active Notes:** Send `GET /api/Notes` to confirm the created note appears in your active list.
8. **Update Note:** Send `PUT /api/Notes/{noteId}` to edit the title, description, or color.
9. **Pin / Archive:** Test `PUT /api/Notes/{noteId}/pin` to toggle pin status and `PUT /api/Notes/{noteId}/archive` to archive the note.
### 3. Media Upload (Cloudinary)
10. **Attach Image:** Send `PUT /api/Notes/{noteId}/image` using `multipart/form-data` with key `image` (upload any image file) to verify Cloudinary upload.
### 4. Tags / Labels Management
11. **Create Label:** Send `POST /api/Labels` with `{ "labelName": "Work" }`. Note down the `labelId`.
12. **Tag Note:** Send `POST /api/Labels/note/{noteId}/attach/{labelId}` to link the label to your note.
13. **Filter by Label:** Send `GET /api/Labels/{labelId}/notes` to verify only notes with this tag are returned.
14. **Untag Note:** Send `DELETE /api/Labels/note/{noteId}/detach/{labelId}` to remove the mapping.
### 5. Reminders & RabbitMQ Asynchronous Processing
15. **Set Reminder:** Send `PUT /api/Notes/{noteId}/reminder` with `{ "reminder": "2026-08-30T18:00:00Z" }`.
16. **Verify RabbitMQ Queue:** Check your terminal output to verify that:
...
17. **Get Reminders:** Send `GET /api/Notes/reminders` to view all notes with upcoming reminders.
18. **Remove Reminder:** Send `DELETE /api/Notes/{noteId}/reminder` to clear the reminder timestamp.
### 6. Trash & Permanent Delete Lifecycle
19. **Soft Delete:** Send `PUT /api/Notes/{noteId}/trash` to move the note to trash.
20. **View Trash:** Send `GET /api/Notes/trash` to confirm the note is in the trash bin (it will no longer appear in `GET /api/Notes`).
21. **Restore Note:** Send `PUT /api/Notes/{noteId}/restore` to move it back to active notes.
22. **Hard Delete:** Send `DELETE /api/Notes/{noteId}/forever` to permanently destroy the note, or `DELETE /api/Notes/trash/empty` to wipe out all trashed notes.